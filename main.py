...
class Application:
    def __init__(self, master, *args, **kwargs):
        ...
        self.master = master
        # frames
        self.left = Frame(master, width=850, height=768, bg='skyblue')
        self.left.pack(side=LEFT)

        self.right = Frame(master, width=700, height=768, bg='lightblue')
        self.right.pack(side=RIGHT) 

        # components
        self.heading = Label(self.left, text="Smart Store", font=('arial 40 bold'), bg='green',fg='red',width=30)
        self.heading.place(x=0, y=0)

        self.date_l = Label(self.right, text="Today's Date: " + str(date), font=('arial 16 bold'), bg='black', fg='orange',width=53)
        ...
        # update button
        self.update_btnid = Label(self.left, text="Update Products in Database", font=('arial 18 bold'), bg='black',fg='orange')
        self.update_btnid.place(x=0, y=100)
        self.update_btn = Button(self.left, text='update', width=22, height=2, bg='blue', fg='yellow', command=self.update_db)
        self.update_btn.place(x=350, y=100)
       
        # add_to_db button
        self.add_to_dbid = Label(self.left, text="Add Products to Database", font=('arial 18 bold'), bg='black',fg='orange')
        self.add_to_dbid.place(x=0, y=170)
        self.add_to_db_btn = Button(self.left, text='add', width=22, height=2, bg='blue', fg='yellow', command=self.add_to_db)
        self.add_to_db_btn.place(x=350, y=170)
       
        # enter stuff
        self.enterid = Label(self.left, text="Enter Product's ID", font=('arial 18 bold'),bg='black',fg='orange')
        self.enterid.place(x=0, y=250)

        self.enteride = Entry(self.left, width=25, font=('arial 18 bold'), bg='white',)
        self.enteride.place(x=250, y=250)
        self.enteride.focus()

        # button
        self.search_btn = Button(self.left, text="Search", width=22, height=2, bg='orange', command=self.ajax)
        self.search_btn.place(x=350, y=300)


        # fill it later by the function ajax
        self.productname = Label(self.left, text="", font=('arial 20 bold'), bg='red',fg='yellow')
        self.productname.place(x=0, y=350)

        self.pprice = Label(self.left, text="", font=('arial 20 bold'), bg='red',fg='yellow')
        self.pprice.place(x=0, y=390)

        # total label
        self.total_l = Label(self.right, text="", font=('arial 40 bold'),bg='black',fg='orange')
    ...
    def update_db(self, *args,**kwargs):
        update_script_path = os.path.join(".", "modules", "update.py")
        os.startfile(update_script_path)

    def add_to_db(self, *args,**kwargs):
        add_to_db_script_path = os.path.join(".", "modules", "add_to_db.py")
        os.startfile(add_to_db_script_path)
    ...
    def ajax(self, *args, **kwargs):
        ...
        self.quantity_l = Label(self.left, text="Enter Quantity", font=('arial 18 bold'),bg='black',fg='orange')
        self.quantity_l.place(x=0, y=450)

        self.quantity_e = Entry(self.left, width=25, font=('arial 18 bold'), bg='white')
        self.quantity_e.place(x=190, y=450)
        self.quantity_e.focus()

        # discount
        self.discount_l = Label(self.left, text="Enter Discount", font=('arial 18 bold'),bg='black',fg='orange')
        self.discount_l.place(x=0, y=500)

        self.discount_e = Entry(self.left, width=25, font=('arial 18 bold'), bg='white')
        self.discount_e.place(x=190, y=500)
        self.discount_e.insert(END, 0)


        # add to cart button
        self.add_to_cart_btn = Button(self.left, text="Add To Cart", width=22, height=2, bg='orange', command=self.add_to_cart)
        self.add_to_cart_btn.place(x=350, y=550)

        # generate bill and change
        self.change_l = Label(self.left, text="Given Amount", font=('arial 18 bold'),bg='black',fg='orange')
        self.change_l.place(x=0, y=600)

        self.change_e = Entry(self.left, width=25, font=('arial 18 bold'), bg='white')
        self.change_e.place(x=190, y=600)

        # button change
        self.change_btn = Button(self.left, text="Calculate Change", width=22, height=2, bg='orange', command=self.change_func)
        self.change_btn.place(x=350, y=650)
    ...
    def change_func(self, *args, **kwargs):
        ...
        # label change
        self.c_amount = Label(self.left, text="Change: Rs. " + str(self.to_give), font=('arial 18 bold'), fg='red', bg='skyblue')
        self.c_amount.place(x=0 , y=650)
    
    def generate_bill(self, *args, **kwargs):
        # create the bill before updating to the database.
        directory = "D:/BILL SYNC/Invoice/" + str(date) + "/"
        if not os.path.exists(directory):
            os.makedirs(directory)
        
        # TEMPLATES FOR THE BILL
        company = "\t\t\t\tBill Sync Pvt. Ltd.\n"
        ...
        result = cur.execute(initial, (product_id[self.x], ))
